...
void Domain::set_initial_box()
{
  ...
  if (boundary[0][0] == BOUND_S) boxlo[0] -= SMALL;
  else if (boundary[0][0] == BOUND_M) minxlo = boxlo[0];
  if (boundary[0][1] == BOUND_S) boxhi[0] += SMALL;
  else if (boundary[0][1] == BOUND_M) minxhi = boxhi[0];

  if (boundary[1][0] == BOUND_S) boxlo[1] -= SMALL;
  else if (boundary[1][0] == BOUND_M) minylo = boxlo[1];
  if (boundary[1][1] == BOUND_S) boxhi[1] += SMALL;
  else if (boundary[1][1] == BOUND_M) minyhi = boxhi[1];

  if (boundary[2][0] == BOUND_S) boxlo[2] -= SMALL;
  else if (boundary[2][0] == BOUND_M) minzlo = boxlo[2];
  if (boundary[2][1] == BOUND_S) boxhi[2] += SMALL;
  else if (boundary[2][1] == BOUND_M) minzhi = boxhi[2];
}
...
void Domain::reset_box()
{
  if (nonperiodic == 2) {
    ...
    if (xperiodic == 0) {
      if (boundary[0][0] == BOUND_S) boxlo[0] = -all[0][0] - SMALL;
      else if (boundary[0][0] == BOUND_M) boxlo[0] = MIN(-all[0][0]-SMALL,minxlo);
      if (boundary[0][1] == BOUND_S) boxhi[0] = all[0][1] + SMALL;
      else if (boundary[0][1] == BOUND_M) boxhi[0] = MAX(all[0][1]+SMALL,minxhi);
      if (boxlo[0] > boxhi[0]) error->all("Illegal simulation box");
    }
    if (yperiodic == 0) {
      if (boundary[1][0] == BOUND_S) boxlo[1] = -all[1][0] - SMALL;
      else if (boundary[1][0] == BOUND_M) boxlo[1] = MIN(-all[1][0]-SMALL,minylo);
      if (boundary[1][1] == BOUND_S) boxhi[1] = all[1][1] + SMALL;
      else if (boundary[1][1] == BOUND_M) boxhi[1] = MAX(all[1][1]+SMALL,minyhi);
      if (boxlo[1] > boxhi[1]) error->all("Illegal simulation box");
    }
    if (zperiodic == 0) {
      if (boundary[2][0] == BOUND_S) boxlo[2] = -all[2][0] - SMALL;
      else if (boundary[2][0] == BOUND_M) boxlo[2] = MIN(-all[2][0]-SMALL,minzlo);
      if (boundary[2][1] == BOUND_S) boxhi[2] = all[2][1] + SMALL;
      else if (boundary[2][1] == BOUND_M) boxhi[2] = MAX(all[2][1]+SMALL,minzhi);
...
void Domain::set_boundary(int narg, char **arg)
{
  ...
  for (int idim = 0; idim < 3; idim++)
    for (int iside = 0; iside < 2; iside++) {
      ...
      if (c == 'p') boundary[idim][iside] = BOUND_P;
      else if (c == 'f') boundary[idim][iside] = BOUND_F;
      else if (c == 's') boundary[idim][iside] = BOUND_S;
      else if (c == 'm') boundary[idim][iside] = BOUND_M;
      else error->all("Illegal boundary command");
    }

  for (int idim = 0; idim < 3; idim++)
    if ((boundary[idim][0] == BOUND_P && boundary[idim][1]) ||
	(boundary[idim][0] && boundary[idim][1] == BOUND_P))
      error->all("Both sides of boundary must be periodic");

  if (boundary[0][0] == BOUND_P) xperiodic = 1;
  else xperiodic = 0;
  if (boundary[1][0] == BOUND_P) yperiodic = 1;
  else yperiodic = 0;
  if (boundary[2][0] == BOUND_P) zperiodic = 1;
  else zperiodic = 0;
  ...
  if (xperiodic == 0 || yperiodic == 0 || zperiodic == 0) {
    nonperiodic = 1;
    if (boundary[0][0] >= BOUND_S || boundary[0][1] >= BOUND_S ||
	boundary[1][0] >= BOUND_S || boundary[1][1] >= BOUND_S ||
	boundary[2][0] >= BOUND_S || boundary[2][1] >= BOUND_S) nonperiodic = BOUND_S;
  }
}
