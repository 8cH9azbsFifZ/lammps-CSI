...
void ComputeAcklandAtom::init()
{
  ...
  chi = (int *) memory->smalloc(8*sizeof(double),"compute/ackland/atom:chi");
}
...
void ComputeAcklandAtom::compute_peratom()
{
  ...
  if (atom->nlocal > nmax) {
    memory->sfree(structure);
    nmax = atom->nmax;
    structure = (double *) memory->smalloc(nmax*sizeof(double),"compute/ackland/atom:ackland");
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      ...
      if (structure[i] == UNKNOWN)
         nUNKNOWN += 1.;
      else if (structure[i] == ICO)
         nICO += 1.;
      else if (structure[i] == BCC)
         nBCC += 1.;
      else if (structure[i] == FCC)
         nFCC += 1.;
      else if (structure[i] == HCP)
         nHCP += 1.;
   ...
   ConcentrationUNKNOWN = ConcentrationICO = ConcentrationBCC = ConcentrationFCC = ConcentrationHCP = 0.;
   MPI_Allreduce (&nUNKNOWN, &ConcentrationUNKNOWN, 1, MPI_DOUBLE, MPI_SUM, world);
   MPI_Allreduce (&nICO, &ConcentrationICO, 1, MPI_DOUBLE, MPI_SUM, world);
   MPI_Allreduce (&nBCC, &ConcentrationBCC, 1, MPI_DOUBLE, MPI_SUM, world);
   MPI_Allreduce (&nFCC, &ConcentrationFCC, 1, MPI_DOUBLE, MPI_SUM, world);
   MPI_Allreduce (&nHCP, &ConcentrationHCP, 1, MPI_DOUBLE, MPI_SUM, world);
   ConcentrationUNKNOWN /= natoms;
   ConcentrationICO /= natoms;
   ConcentrationBCC /= natoms;
   ConcentrationFCC /= natoms;
   ConcentrationHCP /= natoms;
}
