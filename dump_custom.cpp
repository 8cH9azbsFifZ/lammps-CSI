...
enum{TAG,MOL,TYPE,X,Y,Z,XS,YS,ZS,XU,YU,ZU,IX,IY,IZ,
     ...
     LDENS,LTEMP, // local properties
     SMISES, // Von Mises Stress
     STRACE, // Trace of Stress
     SFULL, // Full Norm of Stress
     ACKLAND, // Ackland Method
     COMPUTE};
  ...
  Dump(lmp, narg, arg)
{
  ...
  index_epair = index_ke = index_etotal = index_centro = index_stress = index_ackland = -1;
  ...
  if (index_ackland >= 0) create_compute(style_ackland,NULL);
...
DumpCustom::~DumpCustom()
{
  ...
  if (index_ackland >= 0)  modify->delete_compute(id_compute[index_ackland]);
...
int DumpCustom::count()
{
  ...
  if (nthresh) {
	...
	nstride = 1;
      } else if (thresh_array[ithresh] == ACKLAND) {
   ptr = compute[index_ackland]->scalar_atom;
   nstride = 1;

   // Scalar Stress
   #warning Scalar Stresses are not met by any threshold
	...
	nstride = 6;
      // Local Properties
      } else if (thresh_array[ithresh] == LDENS) {
         //ptr = ((FixLdens *) modify->fix[ifix_ldens])->ldens;
         //nstride = 1;
      } else if (thresh_array[ithresh] == LTEMP) {
         //ptr = ((FixLtemp *) modify->fix[ifix_ltemp])->ltemp;
         //nstride = 1;
...
void DumpCustom::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 5; iarg < narg; iarg++) {
    ...
    } else if (strcmp(arg[iarg],"ackland") == 0) {
      pack_choice[i] = &DumpCustom::pack_ackland;
      vtype[i] = DOUBLE;
      index_ackland = add_compute(style_ackland,1);

    // Scalar Stress
    } else if (strcmp(arg[iarg],"sMises") == 0) {
      pack_choice[i] = &DumpCustom::pack_sMises;
      vtype[i] = DOUBLE;
      index_stress = add_compute(style_stress,1);
    } else if (strcmp(arg[iarg],"sTrace") == 0) {
       pack_choice[i] = &DumpCustom::pack_sTrace;
       vtype[i] = DOUBLE;
       index_stress = add_compute(style_stress,1);
    } else if (strcmp(arg[iarg],"sFull") == 0) {
       pack_choice[i] = &DumpCustom::pack_sFull;
       vtype[i] = DOUBLE;
       index_stress = add_compute(style_stress,1);
...
int DumpCustom::modify_param(int narg, char **arg)
{
  if (strcmp(arg[0],"region") == 0) {
    ...
    // Local Properties
    else if (strcmp(arg[1],"ldens") == 0) thresh_array[nthresh] = LDENS;
    else if (strcmp(arg[1],"ltemp") == 0) thresh_array[nthresh] = LTEMP;
    ...
    } else if (strcmp(arg[1],"ackland") == 0) {
      thresh_array[nthresh] = ACKLAND;
      if (index_ackland < 0) {
   index_ackland = add_compute(style_ackland,1);
   create_compute(style_ackland,NULL);
      }

    // Scalar Stresses
    } else if (strcmp(arg[1],"sMises") == 0) {
      thresh_array[nthresh] = SMISES;
      if (index_stress < 0) {
         index_stress = add_compute(style_stress,1);
         create_compute(style_stress,NULL);
      }
    } else if (strcmp(arg[1],"sTrace") == 0) {
       thresh_array[nthresh] = STRACE;
       if (index_stress < 0) {
          index_stress = add_compute(style_stress,1);
          create_compute(style_stress,NULL);
       }
    } else if (strcmp(arg[1],"sFull") == 0) {
       thresh_array[nthresh] = SFULL;
       if (index_stress < 0) {
          index_stress = add_compute(style_stress,1);
          create_compute(style_stress,NULL);
       }
...
void DumpCustom::pack_ackland(int n)
{
   double *structure = compute[index_ackland]->scalar_atom;
   int nlocal = atom->nlocal;

   for (int i = 0; i < nlocal; i++)
      if (choose[i]) {
         buf[n] = (double)structure[i];
         n += size_one;
      }
}
...
void DumpCustom::pack_sMises(int n) 
{
   double **stress = compute[index_stress]->vector_atom;
   int nlocal = atom->nlocal;
#  define SXX stress[i][0]
#  define SYY stress[i][1]   
#  define SZZ stress[i][2]   
#  define SXY stress[i][3]
#  define SXZ stress[i][4]
#  define SYZ stress[i][5]

   for (int i = 0; i < nlocal; i++)
      if (choose[i]) {
         buf[n] = sqrt ( SXX*SXX + SYY*SYY + SZZ*SZZ + 3.0*(SXY*SXY + SXZ*SXZ + SYZ*SYZ) - (SXX*SYY + SYY*SZZ + SZZ*SXX));
         n += size_one;
      }
}


void DumpCustom::pack_sTrace(int n) 
{
   double **stress = compute[index_stress]->vector_atom;
   int nlocal = atom->nlocal;
   ...
   for (int i = 0; i < nlocal; i++)
      if (choose[i]) {
         buf[n] = sqrt ( SXX*SXX + SYY*SYY + SZZ*SZZ );
         n += size_one;
      }
}


void DumpCustom::pack_sFull(int n) 
{
   double **stress = compute[index_stress]->vector_atom;
   int nlocal = atom->nlocal;
   ...
   for (int i = 0; i < nlocal; i++)
      if (choose[i]) {
         buf[n] = sqrt ( SXX*SXX + SYY*SYY + SZZ*SZZ + SXY*SXY + SXZ*SXZ + SYZ*SYZ );
         n += size_one;
      }
}
...
void DumpCustom::pack_ldens(int n) 
{
   double *ldens = NULL;//((FixLdens *) modify->fix[ifix_ldens])->ldens;
   int nlocal = atom->nlocal;
   for (int i = 0; i < nlocal; i++)
      if (choose[i]) {
         buf[n] = ldens[i];
...
void DumpCustom::pack_ltemp(int n)
{
   double *ltemp = NULL;//((FixLtemp *) modify->fix[ifix_ltemp])->ltemp;
   int nlocal = atom->nlocal;
   for (int i = 0; i < nlocal; i++)
      if (choose[i]) {
         buf[n] = ltemp[i];
